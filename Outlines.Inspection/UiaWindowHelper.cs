...
namespace Outlines.Inspection
{
    public class UiaWindowHelper
    {
        ...
        private enum WindowInfoIndices
        {
            GWL_EXSTYLE = -20,
            GWL_HINSTANCE = -6,
            GWL_HWNDPARENT = -8,
        ...
        private static extern int GetWindowLong(IntPtr hwnd, WindowInfoIndices infoIndex);

        // Based on https://docs.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-setwindowlongw
        [DllImport("user32.dll")]
        private static extern IntPtr SetWindowLong(IntPtr hwnd, WindowInfoIndices infoIndex, int infoValue);

        public void HideWindowFromUia(IntPtr hwnd)
        {
            int windowStyle = GetWindowLong(hwnd, WindowInfoIndices.GWL_EXSTYLE);
            windowStyle |= (int)(ExtendedWindowStyles.WS_EX_LAYERED | ExtendedWindowStyles.WS_EX_TRANSPARENT);
            SetWindowLong(hwnd, WindowInfoIndices.GWL_EXSTYLE, windowStyle);
        }
    }
}
